...
def test_health():
    # `vault_enabled`/`ocr_enabled` (additive, P5.T8/P7.T4) — both off by
    # default, matching VAULT_ENABLED/OCR_ENABLED unset in conftest.py's
    # autouse isolation fixture (and, for OCR, no tesseract/pdftoppm on this
    # test runner's PATH either — see test_ocr.py for the binary-mocked
    # cases).
    assert client.get("/api/health").json() == {
        "status": "ok", "vault_enabled": False, "ocr_enabled": False}


def test_health_reflects_vault_enabled(monkeypatch):
    monkeypatch.setenv("VAULT_ENABLED", "1")
    assert client.get("/api/health").json() == {
        "status": "ok", "vault_enabled": True, "ocr_enabled": False}
