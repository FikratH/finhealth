...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn("px-1.5 py-1 font-mono text-xs uppercase tracking-wide text-ink-muted", className)}
      {...props}
    />
  )
}
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-1.5 py-1.5 pr-8 pl-1.5 font-mono text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
