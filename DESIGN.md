...
The `role="status"` live-region idiom on a terminal confirmation panel now has two call sites — `signin-form.tsx`'s sent state and `pricing/waitlist-form.tsx`'s joined/already-joined state, the latter wrapping an `AnnunciatorCell` rather than a second bezel of its own (nesting one would be the "nested cards" anti-pattern). Only the waitlist form adds programmatic focus on top of that shared idiom: `tabIndex={-1}` plus a `ref.current.focus()` call on mount moves keyboard focus onto the confirmation panel itself, so a keyboard user isn't stranded on `<body>` once the submit button they were focused on unmounts along with the whole form.
